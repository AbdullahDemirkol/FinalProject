...
namespace IdentityCheckServiceApi.Business.Concrete
{
    public class IdentityManager : IIdentityService
    {
        ...
        public IdentityManager(IConfiguration configuration, IdentityCheckContext identityCheckContext, ILogger<IdentityManager> logger)
        {
        ...
        public Task<LoginResponseModel> Login(LoginRequestModel loginRequestModel)
        {
            ...
            _logger.LogInformation($"{user.Username} adlı kullanıcıyla giriş yapıldı.");
        ...
        private List<Claim> SetClaims(UserModel user, LoginRequestModel loginRequestModel)
        {
            ...
            _logger.LogInformation($"{user.Username} adlı kullanıcının bilgileri tokena yerleştirildi.");
            return claims;
        }
        private JwtSecurityToken CreateJwtSecurityToken(TokenOptions tokenOptions, List<Claim> claims, DateTime accessTokenExpiration, SigningCredentials creds)
        {
            ...
            _logger.LogInformation($"Yeni token oluşturuldu.");
            return token;
        ...
        public async Task<string> Register(UserModel user, string roleName)
        {
            try
            {
                ...
                _logger.LogInformation($"{user.Username} adlı kullanıcı kayıt edildi.");
            }
            catch (Exception e)
            {
                _logger.LogInformation($"Kayıt etme işlemi sırasında hata oluştu. Hata Mesajı: {e.Message}");
            }
        ...
        private UserModel Get(Expression<Func<UserModel, bool>> filter, params Expression<Func<UserModel, object>>[] includes)
        {
            ...
            _logger.LogInformation($"İstenilen kullanıcı getirildi.");
            //UserModel user;
            //if (includes!=null)
            //{
        ...
        public async Task<string> SetStatusCancel(int userId, bool userStatus)
        {
            try
            {
                ...
                _logger.LogInformation($"{user.Username} adlı kullanıcının durumu pasif hale getirildi.");
            }
            catch (Exception e)
            {
                _logger.LogError($"Kullanıcı pasif hale getirilirken hata oluştu. Hata Mesajı: {e.Message}");
        ...
        public async Task<bool> ChangeProfilePicture(byte[] userProfilePicture, string userName)
        {
            ...
            if (userProfilePicture != null)
            {
                ...
                if (result == "Kullanıcı Resimi Eklenildi.")
                {
                    _identityCheckContext.Users.Update(user);
                    _identityCheckContext.SaveChanges();
                    _logger.LogInformation($"{user.Username} adlı kullanıcının profil resmi değiştirildi.");
                    return await Task.FromResult(true);
                }
            }
            _logger.LogInformation($"{user.Username} adlı kullanıcının profil resmi değiştirilemedi.");
        ...
        public string GetProfilePicture(string userName)
        {
            ...
            _logger.LogInformation($"{userName} adlı kullanıcının profil resmi getirildi.");
        ...
        public async Task<bool> AddUser(UserModelDTO addUserModel, int roleId)
        {
            ...
            if (addUserModel.ProfilePicture != null)
            {
                ...
                if (result != "Kullanıcı Resimi Eklenildi.")
                {
                    _logger.LogInformation($"{addUserModel.UserModel.Username} adlı yeni kullanıcı eklerken yeni kullanıcı resiminde hata oluştu. Hata Mesajı: {result}.");
                    return await Task.FromResult(false);
                }
                _identityCheckContext.Users.Update(addUserModel.UserModel);
                _identityCheckContext.SaveChanges();
                _logger.LogInformation($"{addUserModel.UserModel.Username} adlı yeni kullanıcı eklenildi.");
                return await Task.FromResult(true);
        ...
        public async Task<bool> UpdateUser(UserModelDTO updateUser, string newPas)
        {
            try
            {
                ...
                if (updateUser.ProfilePicture != null)
                {
                    ...
                    {
                        _logger.LogInformation($"{updateUser.UserModel.Username} adlı kullanıcı güncellenirken kullanıcı resiminde hata oluştu. Hata Mesajı: {result}.");
                        return await Task.FromResult(false);
                    }
                ...
                _identityCheckContext.Users.Update(user);
                _identityCheckContext.SaveChanges();
                _logger.LogInformation($"{updateUser.UserModel.Username} adlı kullanıcı güncellendi.");
                return await Task.FromResult(true);
            }
            catch (Exception e)
            {
                _logger.LogInformation($"{updateUser.UserModel.Username} adlı kullanıcı güncellenirken hata oluştu. Hata Mesajı: {e.Message}");
                return await Task.FromResult(false);
            }
